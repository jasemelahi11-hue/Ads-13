...
async function startApp() {
        ...
        body: JSON.stringify({
           model: "openai/gpt-oss-20b",
            messages: [
                {
                    role: "system",
                    content: "You are a professional marketing and advertising copywriter."
                },
                {
                    role: "user",
                    content: `Write a short and catchy advertising text for the topic "${topic}" with a "${tone || 'attractive'}" tone.`
                }
            ],
            temperature: 0.7
        })
    ...
    if (!data.choices || data.choices.length === 0) {
        ...
        throw new Error(data.error?.message || 'خطا در پاسخ هوش مصنوعی');
    }
    ...
    try {
        ...
        const adText = await generateTextWithGroq(topic, tone);
